...
function processPliLangElements(container = document) {
  ...
  let searchValue = finder && finder.trim() !== "" 
    ? finder.replace(/\\b/g, '') 
    : keyword.replace(/\\b/g, '');

  // Создаем регулярное выражение
  let regex = createRegex(searchValue);

  pliLangElements.forEach((element) => {
    Array.from(element.childNodes).forEach((node) => {
      if (node.nodeType === Node.TEXT_NODE) {
        if (dictionaryVisible) {
          // Если словарь включен, обрабатываем текст узла
          ...
          nodeWords.forEach((word, index) => {
            ...
            // Выделяем слово, если оно совпадает с регулярным выражением
            displayedWord = highlightWord(word, regex);
          ...
          // Если словарь выключен, просто выделяем совпадения
          if (regex && regex.test(node.textContent)) {
            const highlightedText = node.textContent.replace(regex, match => `<b class='match finder'>${match}</b>`);
...
function createRegex(searchValue) {
  if (searchValue !== "") {
    try {
      searchValue = searchValue.replace(/\\b/g, '').replace(/%08/g, '\\b'); // Обрабатываем \b
      return new RegExp(searchValue, 'gi'); // Регулярное выражение для выделения совпадений
    } catch (error) {
      console.error("Ошибка при обработке searchValue:", error);
    }
  }
  return null;
}
...
function highlightWord(word, regex) {
  if (regex && regex.test(word)) {
    return word.replace(regex, match => `<b class='match finder'>${match}</b>`);
  }
  return word;
}
