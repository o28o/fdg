...
function loadStandaloneScripts(lang) {
    return new Promise((resolve, reject) => {
        // Определяем язык: если передан явно - используем его, иначе смотрим путь
        const isRu = lang 
            ? lang === 'ru' 
            : location.pathname.startsWith('/ru/') || location.pathname.startsWith('/r/');
...
document.addEventListener('DOMContentLoaded', function() {
 if (savedDict === "standalonebwru") {
        // Загружаем скрипты standalone-словаря только если выбран standalonebwru
...
if (dictUrl.startsWith("standalonebw")) {
