...
cat $apachesitepath/new/templates/footer | sed 's@WORDREPLACELINK@'"$wordLinkToReplace"'@g' | awk -v config="$(cat $searchBuilderConfiguration)" '{gsub("searchBuilderConfiguration", config )}1' | sed 's@WordToExclude1@'"$WordToExclude1"'@' | sed "s@HIST@"$mainpagebase"/history.php@g" >> $output/${prefix}r.html
