...
if (paliData[segment] !== undefined && transData[segment] !== undefined) {
  html += `${openHtml}<span id="${anchor}"><span class="pli-lang inputscript-ISOPali" lang="pi">${paliData[segment]}</span><span class="rus-lang" lang="ru">${transData[segment]}</span></span>${closeHtml}\n\n`;
...
.catch(error => {
  ...
  suttaArea.innerHTML = `<p>Поиск "${decodeURIComponent(slug)}". Ожидайте.
    ...
    Включены только dn, mn, sn, an, некоторые книги kn и виная<br></p>`;
});
