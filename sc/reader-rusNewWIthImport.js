...
<body class="dark">
            <form id="form" class="ui-widget mb-2">
    ...
    <div id="form-div" class=""> 
       ...
       <a href="/ru/read.php" title="Читать Сутты и Винаю" rel="noreferrer">
    <img class="common-size-icon" alt="page for reading" src="/assets/svg/home-icon.svg"></img>   
         </a>
         </div>
  
       <div id="fdg-button" class="icon-button ">
           <a  href="/ru" title="Искать в Суттах и Винае" rel="noreferrer"
            ><img width="26px" alt="main page for search" src="/assets/img/gray-white.png"></a>
      </div>
        <div id="theme-button" class="icon-button ">
<img src="/assets/svg/circle-half-stroke.svg" alt="Переключить тему" onclick="switchIcon(this)" class="common-size-icon3 changesvg gray-icon"></img>
 </div> 
...
<a id="toggle-mode" href="javascript:void(0);" class="toggle-icon">
  ...
  <img src="/assets/svg/eye.svg" class="common-size-icon4"></img> </a></div>   
...
<script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('/assets/common/common.html')
                ...
                .then(data => {
                    ...
                    // Находим шаблон с общими элементами
                    const template = tempContainer.querySelector('#common-template');

                    // Вставляем содержимое шаблона в нужное место на странице
                    if (template) {
                        document.getElementById('common-content').appendChild(template.content.cloneNode(true));
                    }
                })
                .catch(error => console.error('Ошибка загрузки общих элементов:', error));
        });
    </script>		    
...
<a href="/ru/assets/linebyline.html" class="dynamic-link" title="Create Pali-Russ translation files" rel="noreferrer">
	  ...
	  <script src="/sc/urlForLbl.js"defer></script>
        ...
        <div style="text-align: center;" class="text-muted" id="copyrightnote">
<div class="container"> <small><a target="_blank" rel="license" href="/ru/#copyrightnote">Информация о Правах</a> &copy; Dhamma.gift 2022</a>-<span id="date"></span></small> <button class="btn btn-secondary text-center installButton" id="" style="display:none;"><?php echo $installpwa;?></button></div>
        </div>    
        <script>
  const now = new Date();
