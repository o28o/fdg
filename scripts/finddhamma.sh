...
<tr><td><a class=\"outlink\" href=\"./result/${table}\">${userpattern}</a></td><td><input type='checkbox' data-index=\"${table}\"/></td><td>$textsqnty</td><td>$matchqnty</td><td><a class=\"outlink\" href=\"./result/${tempfilewords}\">$uniqwordtotal</a></td><td>${fortitle^}</td><td>$language</td><td class=\"daterow\">$dateforhist</td><td>`ls -lh ${table} | awk '{print  $5}'`</td><td>" >> $history
