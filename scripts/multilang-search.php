...
if ( preg_match('/-nonl/', $actual_link)) {
  $onl = "";
} else {
    $onl = "-onl";
}
...
$output = shell_exec("bash ./scripts/finddhamma.sh $outputlang $onl $p $string"); 
...
$check = ru2lat( $output );
	 ...
	 $output = shell_exec("bash ./scripts/finddhamma.sh $outputlang $onl -tru $extra $string");
 ...
 $output = trim(preg_replace('/\s\s+/', ' ', $output));	
 $outforjs .= $output . "<br>";
  $output = shell_exec("bash ./scripts/finddhamma.sh $extra $outputlang $onl $convertedStr");
 // echo "<p>$output</p>";
 $output = trim(preg_replace('/\s\s+/', ' ', $output));	
$outforjs .= $output . "<br>";
...
//english 
	  	    ...
	  	    $output = shell_exec("bash ./scripts/finddhamma.sh $outputlang $onl -en $extra $string");
	//		echo "<p>$output</p>";
      $output = trim(preg_replace('/\s\s+/', ' ', $output));	
$outforjs .= $output . "<br>"; 
	  	    ...
	  	    $output = shell_exec("bash ./scripts/finddhamma.sh $outputlang $onl -b $extra $string");
	//		echo "<p>$output</p>";
      $output = trim(preg_replace('/\s\s+/', ' ', $output));	
$outforjs .= $output . "<br>"; 
...
$output = shell_exec("bash ./scripts/finddhamma.sh $outputlang $onl -def -vin $extra $string");
$output = trim(preg_replace('/\s\s+/', ' ', $output));	
$outforjs .= $output . "<br>"; 
}

else if ( preg_match('/-def/', $p ) && ( $p != "-vin" ))  {
$output = shell_exec("bash ./scripts/finddhamma.sh $outputlang $onl -def $extra $string");
//    echo "<p>$output</p>";
$check = ru2lat( $output );
      $output = trim(preg_replace('/\s\s+/', ' ', $output));	
$outforjs .= $output . "<br>"; 
