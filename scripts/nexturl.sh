...
function move {
cd $audiodir
mv /storage/emulated/0/Download/*ogg ./ > /dev/null 2>&1
ls *.ogg 2>/dev/null | awk -F'.' '{print "mkdir "substr($1,0,2)"/"$1" ; mv "$1"* "substr($1,0,2)"/"$1}' | sort -Vf | uniq > tmpmove
bash tmpmove > /dev/null 2>&1
}
...
do
#echo $line 
echo "https://voice.suttacentral.net/scv/index.html?#/sutta?search=$line" >> $audiodir/tmpcheck
done

[[ "$@" != *"-s"* ]] && cat $audiodir/tmpcheck | head -n1  || echo nots
	cd - >/dev/null
}
