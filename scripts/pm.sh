...
if [ "$i" -eq $last ]; then
 ...
 ruletext=$(echo "$line" | grep -v "$type $i" | awk -F'|' '{print $2}' | sed 's/[[:space:]]*$//')
  fullruletext=$(jq -r 'to_entries[] | "\(.key)|\(.value)"' $bupm | sed '/'$type' '$i'\./,/'$type' '$next'\./!d;//d' |awk -F'|' '{print $2}' )
...
<a href="/ru/sc/?q=pli-tv-bu-vb-sk'$i'#pli-tv-bu-vb-sk'$i':'$vbindex'">vb</a>'$fullruletext'
</span>
</div>'
