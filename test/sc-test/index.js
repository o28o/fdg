...
function buildSutta(slug) {
  ...
  console.log("slugReady is " + slugReady, slug); 
...
// ISOPali ISO IASTPali IAST
      ...
      html += `${openHtml}<span class="pli-lang aksa inputscript-ISOPali" lang="pi">${paliData[segment]}</span><span class="eng-lang" lang="en">${transData[segment]}</span>${closeHtml}\n\n`;
    });

      const scButton = `<a href="https://suttacentral.net/${slug}/en/${translator}">Read on SC.net</a>`;
      const scLink = `<p class="sc-link"><a href="/legacy.suttacentral.net/sc/pi/${slug}.html">legacy.SC</a>&nbsp;<a href="https://suttacentral.net/${slug}/en/${translator}">SC.net</a>&nbsp;<a href="https://voice.suttacentral.net/scv/index.html?#/sutta?search=${slug}">Voice.SC</a></p>`; 
...
const translatorCapitalized = translator.charAt(0).toUpperCase() + translator.slice(1);
     const translatorByline = `<div class="byline"><p>Translated by ${translatorCapitalized}</p></div>`;
      suttaArea.innerHTML =  html + translatorByline + scLink;
     ...
     const pageTile = document.querySelector("h1");
      ...
      //console.log('inside', nextArray);
        ...
        ? `<a href="?q=${nextSlug}">${nextSlugPrint} ${nextName}<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="body_1" width="15" height="11">
